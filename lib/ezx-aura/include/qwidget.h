...
public:
    ...
    // Morphing Feature
    void                setMorphMode( const MORPHING_MODE_E morphMode, int winId=0 ); //Set morph mode for current TLW
...
public:
	...
	{
		setMaximumSize(maximumWidth(), maxh);
	};
...
public:
    ...
    //void		grabMouse();// No in ZN5 lib
#ifndef QT_NO_CURSOR
    void		grabMouse( const QCursor & );
#endif
    //void		releaseMouse();// No in ZN5 lib
    void		grabKeyboard();
...
struct QTLWExtra {
    ...
    MORPHING_MODE_E  morphMode;                 // morph mode

    //Orientation flag. Only landscape is considered because it is the special case.
    //Portrait mode is default and dominant. See also get and set methods above
