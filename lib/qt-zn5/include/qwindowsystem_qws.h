...
public:
	...
	//for to improve im's performance
...
#ifndef QT_NO_QWS_FADEBACKGROUND
    static void enableFadeBackground( bool );  
#endif

    // Morphing Feature
    void setMorphMode( const MORPHING_MODE_E morphMode );   // set morph mode of device
    MORPHING_MODE_E getMorphMode() const;   // get morph mode of device
    ...
    enum WindowEvent { Create=0x01, Destroy=0x02, Hide=0x04, Show=0x08,
		       Raise=0x10, Lower=0x20, Geometry=0x40, Active=0x80,
                       Move =0xb0, Name=0x0100};
signals:
    void windowEvent( QWSWindow *w, QWSServer::WindowEvent e );
    void requestedSetMorphMode( MORPHING_MODE_E modeId );
    void requestedOrientationMode(bool mode);
#ifndef QT_NO_COP
    ...
    void callStateHideAvailable(bool state);
...
private:
    ...
    // TV Out orientation Feature
    void invokeSetOrientationLandscapeMode( const QWSSetOrientationLandscapeModeCommand *cmd, QWSClient*);
...
private:
    ...
    // Morphing Feature
    MORPHING_MODE_E morphMode;  // morph mode of device
