/****************************************************************************
** $Id: qt/src/tools/qstring.h   2.3.8   edited 2004-08-05 $
**
...
struct Q_EXPORT QStringData : public QShared {
    QStringData() :
	something(0), unicode(0), ascii(0), len(0), maxl(0), dirtyascii(0) { ref(); }
    QStringData(QChar *u, uint l, uint m) :
	something(0), unicode(u), ascii(0), len(l), maxl(m), dirtyascii(0) { }

    ~QStringData() { if ( unicode ) delete[] ((char*)unicode);
                     if ( ascii ) delete[] ascii; }

    void deleteSelf();
    unsigned int something; //4 := 0
    QChar *unicode;	// 8
    char *ascii;	// 12
    uint len;		// 16
    uint maxl:30;	// 20
