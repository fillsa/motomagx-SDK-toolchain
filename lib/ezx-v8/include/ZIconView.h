#ifndef __ZICONVIEW_H__
#define __ZICONVIEW_H__
...
public:
    ...
    MARKTYPE getItemType() const {return (MARKTYPE)markType;}
    ...
    void setView(ZIconView* newView) {view = newView;}
...
public:
    ...
    bool enabled() const ;
     ...
     void setPixmapRectToMax(bool max = TRUE) {pixRectExtend = max;}
...
public:
    ...
    void checkItem(int index, bool select);
     ...
     void removeItem(int index){takeItem(item(index));}
     void setDrmImage(const QPixmap& drm);
